...
echo ssh -o StrictHostKeyChecking=no -i ssh_keys/gse_admin opc@${gse_admin} 'ssh -o StrictHostKeyChecking=no -i '${gse_admin_stagedir}'/'${identity_domain}' opc@'${source_ip}' "cd '${gse_admin_stagedir}' && sudo change_fqdn.sh '${targetIpHOST}' && sudo unzip -o p22336899_R12_GENERIC.zip && sudo chown oracle:dba -R '${gse_admin_stagedir}' && sudo chmod 777 -R '${gse_admin_stagedir}' && cp RemoteClone_v1.7/cln.props RemoteClone_v1.7/cln.props.ORIG && cp cln.props RemoteClone_v1.7/cln.props"'
	ssh -o StrictHostKeyChecking=no -i ssh_keys/gse_admin opc@${gse_admin} 'ssh -o StrictHostKeyChecking=no -i '${gse_admin_stagedir}'/'${identity_domain}' opc@'${source_ip}' "cd '${gse_admin_stagedir}' && sudo change_fqdn.sh '${targetIpHOST}' && sudo unzip -o p22336899_R12_GENERIC.zip && sudo chown oracle:dba -R '${gse_admin_stagedir}' && sudo chmod 777 -R '${gse_admin_stagedir}' && cp RemoteClone_v1.7/cln.props RemoteClone_v1.7/cln.props.ORIG && cp cln.props RemoteClone_v1.7/cln.props"'
