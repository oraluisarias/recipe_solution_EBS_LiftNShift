...
tools_ip=$3
executionPath=$4
sourceIpHOST="oc-"`echo "$source_ip" | tr . -`
...
echo ssh -o StrictHostKeyChecking=no -i ssh_keys/gse_admin opc@${gse_admin} 'ssh -o StrictHostKeyChecking=no -i '${gse_admin_stagedir}'/'${identity_domain}' opc@'${source_ip}' "cd '${gse_admin_stagedir}' && sudo dos2unix '${gse_admin_stagedir}'/change_fqdn.sh && sudo chmod 777 '${gse_admin_stagedir}'/oracle_ebs_workshop.sh && sudo '${gse_admin_stagedir}'/change_fqdn.sh '${sourceIpHOST}' && sudo unzip -o p22336899_R12_GENERIC.zip && sudo chown oracle:dba -R '${gse_admin_stagedir}' && sudo chmod 777 -R '${gse_admin_stagedir}' && cp RemoteClone_v1.7/cln.props RemoteClone_v1.7/cln.props.ORIG && cp cln.props RemoteClone_v1.7/cln.props"'
	ssh -o StrictHostKeyChecking=no -i ssh_keys/gse_admin opc@${gse_admin} 'ssh -o StrictHostKeyChecking=no -i '${gse_admin_stagedir}'/'${identity_domain}' opc@'${source_ip}' "cd '${gse_admin_stagedir}' && sudo dos2unix '${gse_admin_stagedir}'/change_fqdn.sh && sudo chmod 777 '${gse_admin_stagedir}'/oracle_ebs_workshop.sh && sudo '${gse_admin_stagedir}'/change_fqdn.sh '${sourceIpHOST}' && sudo unzip -o p22336899_R12_GENERIC.zip && sudo chown oracle:dba -R '${gse_admin_stagedir}' && sudo chmod 777 -R '${gse_admin_stagedir}' && cp RemoteClone_v1.7/cln.props RemoteClone_v1.7/cln.props.ORIG && cp cln.props RemoteClone_v1.7/cln.props"'
